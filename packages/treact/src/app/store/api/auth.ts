...
import { account$password } from 'store/mtproto.h'
...
function getPassword() {
  const onDone = ({ current_salt }: account$password) =>
    GET_PASSWORD.DONE({
  ...
  return (dispatch: Dispatch) => {
    ...
    return (
      api('account.getPassword', {}, options)
...
export function signIn(phoneCode: string) {
  return (dispatch: Dispatch, getState: () => Store) => {
    const catchNeedPass = (err: AuthError) =>
      err.message === 'SESSION_PASSWORD_NEEDED' ? dispatch(getPassword()) : err
    const catchAndDispatch = pipe(
      tap(catchNeedPass),
      err => dispatch(SIGN_IN.FAIL(err)),
    )
    const { auth } = getState()
...
export function sendCode(phoneNumber: string) {
  const onDone = ({ phone_code_hash }: { phone_code_hash: string }) =>
    SEND_CODE.DONE({
