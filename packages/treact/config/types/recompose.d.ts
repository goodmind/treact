...
declare module 'recompose' {
  ...
  type HandleCreators<TOutter> = {
    [K in keyof TOutter]?: mapper<TOutter, TOutter[K]>;
  };
  ...
  type StateUpdaters<State, TOutter> = {
    [K in keyof TOutter]?: (state: State, props: TOutter) => TOutter[K];
  };
  export function withStateHandlers<State, TOutter, THandlers>(
    initialState: ((props: TOutter) => State) | State,
    stateUpdaters: StateUpdaters<State, TOutter>,
  ): InferableComponentEnhancerWithProps<THandlers, TOutter>;
  ...
  type Compose = (<A, B, C, D, E, F, G, H, I>(
    hi: UnaryFn<H, I>,
    gh: UnaryFn<G, H>,
    fg: UnaryFn<F, G>,
    ef: UnaryFn<E, F>,
